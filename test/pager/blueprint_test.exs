defmodule Pager.BlueprintTest do
  ...
  describe "default provider" do
    ...
    test "enables the first page when current page it's not the first one" do
      blueprint = Pager.Blueprint.explain(%Pager.Page{current_page: 2, total_pages: 5})
    ...
    test "disables the last page when current page it's already the last one" do
      blueprint = Pager.Blueprint.explain(%Pager.Page{current_page: 5, total_pages: 5})
    ...
    test "show outer window to the right if inner window is long enough" do
      blueprint = Pager.Blueprint.explain(%Pager.Page{current_page: 1, total_pages: 5}, inner_window: 1, outer_window: 1)
      assert [%{type: :first}, %{type: :prev}, _, _, %{type: :ellipsis}, _, %{type: :next}, %{type: :last}] = blueprint
    end

    test "show outer window to the left if inner window is long enough" do
      blueprint = Pager.Blueprint.explain(%Pager.Page{current_page: 5, total_pages: 5}, inner_window: 1, outer_window: 1)
      assert [%{type: :first}, %{type: :prev}, _, %{type: :ellipsis}, _, _, %{type: :next}, %{type: :last}] = blueprint
    end

    test "don't show outer window if inner window is too short" do
      blueprint = Pager.Blueprint.explain(%Pager.Page{current_page: 1, total_pages: 5}, outer_window: 2)
      assert [_,_, %{type: page}, %{type: page}, %{type: page}, %{type: page}, %{type: page},_,_] = blueprint
    end
