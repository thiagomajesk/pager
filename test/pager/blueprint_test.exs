defmodule Pager.BlueprintTest do
  ...
  describe "default provider" do
    test "disables the first page when current page it's already the first one" do
      blueprint = Pager.Providers.Default.explain(%Pager.Page{current_page: 1, total_pages: 5})
    ...
    test "enables the first page when current page it's not the first one" do
      blueprint = Pager.Providers.Default.explain(%Pager.Page{current_page: 2, total_pages: 5})
    ...
    test "disables the last page when current page it's already the last one" do
      blueprint = Pager.Providers.Default.explain(%Pager.Page{current_page: 5, total_pages: 5})
    ...
    test "enables the last page when current page it's not the last one" do
      blueprint = Pager.Providers.Default.explain(%Pager.Page{current_page: 4, total_pages: 5})
    ...
    test "show outer window to the right if inner window is long enough" do
      blueprint =
        Pager.Blueprint.explain(%Pager.Page{current_page: 1, total_pages: 5},
          inner_window: 1,
          outer_window: 1
        )
    ...
    test "show outer window to the left if inner window is long enough" do
      blueprint =
        Pager.Blueprint.explain(%Pager.Page{current_page: 5, total_pages: 5},
          inner_window: 1,
          outer_window: 1
        )
    ...
    test "don't show outer window if inner window is too short" do
      blueprint =
        Pager.Blueprint.explain(%Pager.Page{current_page: 1, total_pages: 5}, outer_window: 2)
    ...
    test "when outer window is disabled, don't show ellipsis" do
      blueprint = Pager.Blueprint.explain(%Pager.Page{current_page: 1, total_pages: 5})
  ...
  describe "compact provider" do
    test "disables the first page when current page it's already the first one" do
      blueprint = Pager.Providers.Compact.explain(%Pager.Page{current_page: 1, total_pages: 5})
      assert [%{states: [:disabled]}] = Enum.filter(blueprint, &(&1.type == :first))
    end

    test "enables the first page when current page it's not the first one" do
      blueprint = Pager.Providers.Compact.explain(%Pager.Page{current_page: 2, total_pages: 5})
      assert [%{states: []}] = Enum.filter(blueprint, &(&1.type == :first))
    end
    ...
    test "disables the last page when current page it's already the last one" do
      blueprint = Pager.Providers.Compact.explain(%Pager.Page{current_page: 5, total_pages: 5})
      assert [%{states: [:disabled]}] = Enum.filter(blueprint, &(&1.type == :last))
    end

    test "enables the last page when current page it's not the last one" do
      blueprint = Pager.Providers.Compact.explain(%Pager.Page{current_page: 4, total_pages: 5})
      assert [%{states: []}] = Enum.filter(blueprint, &(&1.type == :last))
    end
    ...
    test "enables the next page when it's relevant" do
      blueprint = Pager.Blueprint.explain(%Pager.Page{current_page: 4, total_pages: 5})
      assert [%{states: []}] = Enum.filter(blueprint, &(&1.type == :next))
    end
  end
end
